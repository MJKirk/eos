...
namespace eos
{
    ...
    BsToPhiDileptonAmplitudes<tag::BFS2004>::BsToPhiDileptonAmplitudes(const Parameters & p,
        ...
        f_Bs(p["decay-constant::B_s"], *this),
        f_phi_par(p["B_s->phi::f_phi_par"], *this),
        f_phi_perp(p["B_s->phi::f_phi_perp@2GeV"], *this),
        lambda_B_p_inv(p["B_s::1/lambda_B_p"], *this),
        a_1_par(p["phi::a_1_para@1GeV"], *this),
        a_2_par(p["phi::a_2_para@1GeV"], *this),
        a_1_perp(p["phi::a_1_perp@1GeV"], *this),
        a_2_perp(p["phi::a_2_perp@1GeV"], *this),
        uncertainty_para(p["B_s->phill::A_para_uncertainty@LargeRecoil"], *this),
        uncertainty_perp(p["B_s->phill::A_perp_uncertainty@LargeRecoil"], *this),
    ...
    BsToPhiDileptonAmplitudes<tag::BFS2004>::dipole_form_factors(const double & s, const WilsonCoefficients<BToS> & wc) const
    {
        ...
        static const double e_s = -1.0/3.0;
        ...
        QCDFIntegrals<BToKstarDilepton> qcdf_0 = this->qcdf_dilepton_massless_case(s, m_B, m_V, mu, a_1_perp, a_2_perp, a_1_par, a_2_par);
        QCDFIntegrals<BToKstarDilepton> qcdf_c = this->qcdf_dilepton_charm_case(s, m_c_pole, m_B, m_V, mu, a_1_perp, a_2_perp, a_1_par, a_2_par);
        ...
        complex<double> Delta_T_ann_top_perp = e_s * M_PI * M_PI * f_Bs / 3.0 / m_b_PS / m_B * (
                ...
                + 2.0 * f_phi_par * (
                        ...
                        + 4.0/3.0 * wc.c4() + 16.0 * wc.c5() + 64.0/3.0 * wc.c6()
                        + 12.0 * wc.c3() + 120.0 * wc.c5() // TODO CHECK
                    ) * (m_V / (1.0 - s / (m_B * m_B)) * lambda_B_p_inv));
        complex<double> Delta_T_ann_up_perp = -e_s * 2.0 * M_PI * M_PI * f_Bs * f_phi_par / 3.0 / m_b_PS / m_B *
            (m_V / (1.0 - s / (m_B * m_B)) * lambda_B_p_inv) * (
                    - 4.0 / 3.0 * wc.c1() - wc.c2() // TODO CHECK
                );
        ...
        complex<double> Delta_T_hsa_top_perp = e_s * a_mu_f * (M_PI * M_PI * f_Bs / (3.0 * m_b_PS * m_B)) * (
                ...
                - (4.0 * m_V * f_phi_par / (1.0 - s / (m_B * m_B)) * lambda_B_p_inv) * (3.0 / 4.0) * (
        ...
        complex<double> Delta_T_hsa_up_perp = e_s * a_mu_f * (M_PI * M_PI * f_Bs / (3.0 * m_b_PS * m_B)) * (
                + 8.0 * f_phi_perp * (3.0 / 4.0) * (wc.c2() - wc.c1() / 6.0) * (qcdf_c.j5_perp - qcdf_0.j5_perp)
                - (4.0 * m_V * f_phi_par / (1.0 - s / (m_B * m_B)) * lambda_B_p_inv) * (3.0 / 4.0) * (wc.c2() - wc.c1() / 6.0)
        ...
        result.calT_perp_right = xi_perp(s) * C_perp_right
            + power_of<2>(M_PI) / 3.0 * (f_Bs * f_phi_perp) / m_B * T_perp_right
            + Delta_T_perp;
        result.calT_parallel = xi_par(s) * C_par
            + power_of<2>(M_PI) / 3.0 * (f_Bs * f_phi_par * m_V) / (m_B * energy) * T_par;

        return result;
    }

    double
    BsToPhiDileptonAmplitudes<tag::BFS2004>::xi_perp(const double & s) const
    {
        const double factor = m_B() / (m_B() + m_V());
    ...
    double
    BsToPhiDileptonAmplitudes<tag::BFS2004>::xi_par(const double & s) const
    {
        const double factor1 = (m_B() + m_V()) / (2.0 * energy(s));
        const double factor2 = (1.0 - m_V() / m_B());
    ...
    double
    BsToPhiDileptonAmplitudes<tag::BFS2004>::norm(const double & s) const
    {
    ...
    double
    BsToPhiDileptonAmplitudes<tag::BFS2004>::mu_f() const
    {
        return 1.5;
    }

    double
    BsToPhiDileptonAmplitudes<tag::BFS2004>::m_b_PS() const
    {
    ...
    BsToPhiDileptonAmplitudes<tag::BFS2004>::amplitudes(const double & s) const
    {
        ...
        const double prefactor_long = -norm_s / (2.0 * m_V() * std::sqrt(s));
        ...
        const double
            ...
            kin_tensor_1 = norm_s / m_V() * ((m_B2 + 3.0 * m_K2 - s) * ff_T2 - lambda(s) / m2_diff * ff_T3),
    ...
    double
    BsToPhiDileptonAmplitudes<tag::BFS2004>::imag_C9_perp(const double & s) const
    {
        WilsonCoefficients<BToS> wc = model->wilson_coefficients_b_to_s(mu(), lepton_flavor, cp_conjugate);
    ...
    double
    BsToPhiDileptonAmplitudes<tag::BFS2004>::real_C9_para(const double & s) const
    {
        WilsonCoefficients<BToS> wc = model->wilson_coefficients_b_to_s(mu(), lepton_flavor, cp_conjugate);
    ...
    double
    BsToPhiDileptonAmplitudes<tag::BFS2004>::imag_C9_para(const double & s) const
    {
        WilsonCoefficients<BToS> wc = model->wilson_coefficients_b_to_s(mu(), lepton_flavor, cp_conjugate);
